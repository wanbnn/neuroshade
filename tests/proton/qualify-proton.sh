...
while [ "$#" -gt 0 ]; do
    case "$1" in
        ...
        *) echo "usage: qualify-proton.sh --dx11-launcher FILE --dx12-launcher FILE [--output FILE]" >&2; exit 64;;
    esac
done
...
qualified=no
[ "$dx11_result" -eq 0 ] && [ "$dx12_result" -eq 0 ] &&
    [ "$dx11_evidence" = yes ] && [ "$dx12_evidence" = yes ] &&
    [ "$dx11_layer_evidence" = yes ] && [ "$dx12_layer_evidence" = yes ] &&
    [ "$gpu_evidence" = yes ] && qualified=yes

temporary="$output.tmp.$$"
printf '{"schema_version":1,"qualified":"%s","proton":"%s","dxvk":"%s","vkd3d_proton":"%s","expected_gpu":"%s","gpu_evidence":"%s","dx11_evidence":"%s","dx12_evidence":"%s","dx11_layer_evidence":"%s","dx12_layer_evidence":"%s","dx11_exit":%s,"dx12_exit":%s,"dx11_log":"%s","dx12_log":"%s"}\n' \
    ...
    "$dx11_result" "$dx12_result" "$dx11_log" "$dx12_log" > "$temporary"
mv "$temporary" "$output"
cat "$output"
[ "$qualified" = yes ]
